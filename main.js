var stateGenerator = (function () {
    ...
    stateMaker$1.prototype.states = function () {
        return this._states;
    };
    ...
    stateMaker$1.prototype.divideCountry = function (seeds, options) {
        ...
        console.debug(options.prob || this.prob);

        var states = seeds.map(this.stateOf, this),
            seedlings = true,
            englarge = function englarge(state) {
            return _this3.enlargeState(state, options);
        };
        ...
        while (seedlings === true) {
            seedlings = states.map(englarge).some(function (d) {
                return d;
            });
        } // Add unselected counties to neighboring states
        if (options.assignOrphans) while (this.unselected.size > 0) {
            Array.from(this.unselected).forEach(this.assignOrphans, this);
        } else console.debug('not assigning orphans');
        return this._states;
    };

    stateMaker$1.prototype.enlargeState = function (state, options) {
    ...
    stateMaker$1.prototype.pickCounty = function (countySet, options) {
        var _this4 = this;
        ...
        if (Math.random() < probability) {
            ...
            var currentNeighbors = neighborlings.filter(function (d) {
    ...
    var evCount = function evCount(statemaker, options) {
        options = options || { reps: 436 };
        return apportion$1(statemaker.states().map(function (state) {
            return statemaker.sum(state, 'pop');
        }), options).map(function (d) {
            return d + 2;
        });
    };

    var apportion_2 = apportion$1;

    var __moduleExports$1 = {
        evCount: evCount,
        apportion: apportion_2
    };
    ...
    var populationScale = d3.scalePow().exponent(2).domain([100000, 10e6]).clamp(true).range([0, 1]);
    ...
    function program(error, topo, csv) {
        ...
        maker.freezeState(dc).divideCountry(seedindices, { assignOrphans: true });
        ...
        var voteCount = function voteCount(key) {
            return maker.states().map(function (state) {
